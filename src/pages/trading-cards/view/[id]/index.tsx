...
import { getTradingCardById } from 'apiSdk/trading-cards';
import { Error } from 'components/error';
import { TradingCardInterface } from 'interfaces/trading-card';
import useSWR from 'swr';
import { useRouter } from 'next/router';
import { AccessOperationEnum, AccessServiceEnum, useAuthorizationApi, withAuthorization } from '@roq/nextjs';
import { deleteCollectionCardById } from 'apiSdk/collection-cards';

function TradingCardViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<TradingCardInterface>(
    ...
    () =>
      getTradingCardById(id, {
        relations: ['user', 'collection_card'],
      }),
  );

  const collection_cardHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('trading_card', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/trading-cards/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('collection_card', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/collection-cards/create?card_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.collection_card?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/collection-cards/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess('collection_card', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/collection-cards/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('collection_card', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    collection_cardHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
